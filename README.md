...
Bundles a default static (non-variable) font, [Space Mono](https://github.com/googlefonts/spacemono) (OFL-licensed). Pass `fonts` in the options to replace it, or spread the public `defaultFonts()` export to keep Space Mono and add another font alongside it:

```ts
import { readFile } from 'node:fs/promises';
import { generateCard, defaultFonts } from '@vdaluz/astro-og-cards';

const fonts = [
  ...(await defaultFonts()),
  { name: 'Fraunces', data: await readFile('fraunces-700.woff'), weight: 700, style: 'normal' },
];

const png = await generateCard(inlineStyledHtml, { fonts });
```

`defaultFonts()` returns the bundled Space Mono regular (400) and bold (700) entries, decoded once per process. It is part of the package's public API. Every font you pass must be static, not variable (see Known gotchas below).
